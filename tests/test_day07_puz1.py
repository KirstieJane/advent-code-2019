from day07.day07_puz1 import (run_opcode, load_computer_data, parse_opcode,
                              apply_opcode1, apply_opcode2,
                              apply_opcode3, apply_opcode4,
                              apply_opcode5, apply_opcode6,
                              apply_opcode7, apply_opcode8,
                              run_code_through_five_amps,
                              find_best_phase_setting_seq,
                              ForbiddenValueError)
import pytest
...
def test_puzzle_ex_0():
    ...
    original_code = [3, 15, 3, 16, 1002, 16, 10, 16, 1, 16, 15, 15, 4, 15, 99,
                     0, 0]

    phase_setting_seq = find_best_phase_setting_seq(original_code,
                                                    amp_input=amp_input)

    assert phase_setting_seq == [4, 3, 2, 1, 0]

    output = run_code_through_five_amps(original_code,
                                        phase_setting_seq,
                                        amp_input=amp_input)

    assert output == 43210, ("Should be 43210")
...
def test_puzzle_ex_1():
    ...
    original_code = [3, 23, 3, 24, 1002, 24, 10, 24, 1002, 23, -1, 23, 101, 5,
                     23, 23, 1, 24, 23, 23, 4, 23, 99, 0, 0]

    phase_setting_seq = find_best_phase_setting_seq(original_code,
                                                    amp_input=amp_input)

    assert phase_setting_seq == [0, 1, 2, 3, 4]

    output = run_code_through_five_amps(original_code,
                                        phase_setting_seq,
                                        amp_input=amp_input)

    assert output == 54321, ("Should be 54321")
...
def test_puzzle_ex_2():
    ...
    original_code = [3, 31, 3, 32, 1002, 32, 10, 32, 1001, 31, -2, 31, 1007,
    ...
    assert phase_setting_seq == [1, 0, 4, 3, 2]

    output = run_code_through_five_amps(original_code,
                                        phase_setting_seq,
                                        amp_input=amp_input)

    assert output == 65210, ("Should be 65210")


def test_puzzle_ex_day5_0():
...
def test_apply_opcode3():
    ...
    code_list = apply_opcode3(code_list,
                              opcode_loc,
                              programme_input=programme_input[0])
